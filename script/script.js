...
const getDataFromGraphql = async(token)=> {
    ...
    welcomeMessage.innerHTML = "Welcome, " + "<br>" +
    "NAME: " + firstName + " " + lastName + "<br>" +
    "PHONE: " + phone + "<br>" +
    "EMAIL: " + mail + "<br>" + "<br>" + "<br>" +
    "RECEIVED XPs:  " + totalAmount;
...
function GeneratePieGraph(doneInt, receivedInt) {
  ...
  const colors = ["transparent", "#9DB089"]; // Using transparent for one part for a visual effect
  let startAngle = 0;
...
function BuildColumnGraphData(data) {
  ...
  data.forEach((point, index) => {
    ...
    column.setAttribute("fill", "#9DB089"); 
